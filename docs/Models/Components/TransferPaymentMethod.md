# TransferPaymentMethod

A method of moving money


## Supported Types

### `Components\MoovWalletTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\MoovWalletTransferPaymentMethod
*/
Components\MoovWalletTransferPaymentMethod $value = /* values here */
```

### `Components\AchDebitFundTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\AchDebitFundTransferPaymentMethod
*/
Components\AchDebitFundTransferPaymentMethod $value = /* values here */
```

### `Components\AchDebitCollectTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\AchDebitCollectTransferPaymentMethod
*/
Components\AchDebitCollectTransferPaymentMethod $value = /* values here */
```

### `Components\AchCreditStandardTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\AchCreditStandardTransferPaymentMethod
*/
Components\AchCreditStandardTransferPaymentMethod $value = /* values here */
```

### `Components\AchCreditSameDayTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\AchCreditSameDayTransferPaymentMethod
*/
Components\AchCreditSameDayTransferPaymentMethod $value = /* values here */
```

### `Components\RtpCreditTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\RtpCreditTransferPaymentMethod
*/
Components\RtpCreditTransferPaymentMethod $value = /* values here */
```

### `Components\CardPaymentTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\CardPaymentTransferPaymentMethod
*/
Components\CardPaymentTransferPaymentMethod $value = /* values here */
```

### `Components\PushToCardTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\PushToCardTransferPaymentMethod
*/
Components\PushToCardTransferPaymentMethod $value = /* values here */
```

### `Components\PullFromCardTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\PullFromCardTransferPaymentMethod
*/
Components\PullFromCardTransferPaymentMethod $value = /* values here */
```

### `Components\ApplePayTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\ApplePayTransferPaymentMethod
*/
Components\ApplePayTransferPaymentMethod $value = /* values here */
```

### `Components\CardPresentPaymentTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\CardPresentPaymentTransferPaymentMethod
*/
Components\CardPresentPaymentTransferPaymentMethod $value = /* values here */
```

### `Components\InstantBankCreditTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\InstantBankCreditTransferPaymentMethod
*/
Components\InstantBankCreditTransferPaymentMethod $value = /* values here */
```

### `Components\PushToApplePayTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\PushToApplePayTransferPaymentMethod
*/
Components\PushToApplePayTransferPaymentMethod $value = /* values here */
```

### `Components\PullFromApplePayTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\PullFromApplePayTransferPaymentMethod
*/
Components\PullFromApplePayTransferPaymentMethod $value = /* values here */
```

### `Components\GooglePayTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\GooglePayTransferPaymentMethod
*/
Components\GooglePayTransferPaymentMethod $value = /* values here */
```

### `Components\PushToGooglePayTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\PushToGooglePayTransferPaymentMethod
*/
Components\PushToGooglePayTransferPaymentMethod $value = /* values here */
```

### `Components\PullFromGooglePayTransferPaymentMethod`

```php
/**
* @var \Moov\MoovPhp\Models\Components\PullFromGooglePayTransferPaymentMethod
*/
Components\PullFromGooglePayTransferPaymentMethod $value = /* values here */
```

